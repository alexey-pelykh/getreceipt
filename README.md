...
> The six bundled sources (`grandfrais.com`, `monoprix.fr`, `free.fr` — Free residential/Freebox, `pro.free.fr` — Free Pro, `particuliers.alpiq.fr`, `amazon.com` — Amazon orders via an imported browser session; a multi-marketplace source whose `amazon.fr` instance is the working marketplace today) are currently
