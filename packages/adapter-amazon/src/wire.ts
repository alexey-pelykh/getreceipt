 ...
 * NOT plaintext. So the contract reads ONLY language-independent structure: the slot-id order ids, the numeric
 * order count, and the `year-YYYY` time filter. The order date is not available at list time — the adapter
 * assigns a coarse issued instant from the filter year instead.
 *
