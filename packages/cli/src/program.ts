...
/**
 * Assemble the root `getreceipt` program: the `from` and `config` verbs, `--version`
 * (which prints the version AND the unofficial-use disclaimer), and a help footer
 * carrying the disclaimer + personal-use posture on every command (`afterAll`), so the
