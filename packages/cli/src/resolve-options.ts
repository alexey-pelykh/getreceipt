...
/**
 * Add the global `--config` / `-p, --profile` options to a command. Applied to the root program and
 * every subcommand: a subcommand's own copy lets the flag appear after the verb, and
...
/**
 * Merge the global `--config`/`--profile` from a command and ALL its ancestors, root→leaf, so the
 * value written closest to the verb wins (`getreceipt --profile a from x --profile b` → `b`).
 * Reads each command's parsed `opts()`; only the two global keys are projected (a verb's own
 * `--out`/`--json`/etc. are never folded in).
 */
export function resolveGlobalOptions(command: Command): GlobalConfigOptions {
    ...
    const merged: { config?: string; profile?: string } = {};
    for (let i = chain.length - 1; i >= 0; i--) {
        const opts = chain[i]?.opts<{ config?: string; profile?: string }>();
 ...
 * and {@link buildConfigSelection} (precedence + divergence warnings).
 */
export function resolveConfigSelection(
    command: Command,
    context: { readonly env?: Record<string, string | undefined>; readonly stderr?: (message: string) => void } = {},
): ConfigSelection {
    return buildConfigSelection(resolveGlobalOptions(command), context);
}
